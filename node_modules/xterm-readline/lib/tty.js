...
class Tty {
    constructor(col, row, tabWidth, out, anchorRow = 0) {
    ...
    refreshLineInner(prompt, line, oldLayout, newLayout, highlighter) {
        var _a, _b;
        const oldScroll = (_a = oldLayout.scrollOffset) !== null && _a !== void 0 ? _a : 0;
        const newScroll = (_b = newLayout.scrollOffset) !== null && _b !== void 0 ? _b : 0;
        ...
        const highlighted = highlighter.highlightPrompt(prompt) +
        ...
        if (desiredVisible > currentBelowAnchor && this.anchorRow > 0) {
            const scrollUp = Math.min(desiredVisible - currentBelowAnchor, this.anchorRow);
            const downBy = this.row - 1 - physicalRow;
            if (downBy > 0)
                this.write(`\x1b[${downBy}B`);
            this.write("\n".repeat(scrollUp));
        ...
        if (allRows.length <= viewport)
            effectiveScroll = 0;
        effectiveScroll = Math.max(0, Math.min(effectiveScroll, allRows.length - viewport));
